...
def main(args):
    ...
    train_data_generator = video_data_generator('data/flow_train_val_split/train_batches/',
                                                batch_size=batch_size,
                                                shuffle=False,
                                                center_crop=True,
                                                two_channels_frame=True)

    val_data_generator = video_data_generator('data/flow_train_val_split/validation_batches/',
                                              batch_size=batch_size,
                                              shuffle=False,
                                              center_crop=True,
                                              two_channels_frame=True)
    ...
    if args.eval_type == 'both' or args.eval_type == 'validation':
        ...
        with open('flow_val_predictions.txt', 'w') as f:
    ...
    if args.eval_type == 'both' or args.eval_type == 'train':
        ...
        with open('flow_train_predictions.txt', 'w') as f:
            for i in np.arange(len(cache_preds)):
